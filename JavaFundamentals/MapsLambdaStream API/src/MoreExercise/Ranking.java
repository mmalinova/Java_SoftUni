...
public class Ranking {
    public static void main(String[] args) {
            ...
            if (contests.containsKey(contest)) {
                if (contests.get(contest).equals(password)) {
                    ...
                    if (!users.get(userName).containsKey(contest)) {
                        ...
                        if (userPoints > users.get(userName).get(contest)) {
                            users.get(userName).put(contest, userPoints);
                        }
        ...
        for (Map.Entry<String, Map<String, Integer>> user : users.entrySet()) {
            if (user.getValue().values().stream().mapToInt(Integer::intValue).sum() == totalSum) {
        ...
        users.forEach((key, value) -> {
            ...
            value.entrySet().stream()
                    .sorted((v1, v2) -> Integer.compare(v2.getValue(), v1.getValue()))//s.getValue() - f.getValue()).
